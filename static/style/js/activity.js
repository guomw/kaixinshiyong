...
var goods = {
	...
	quick_order_return : function(o, type, state, page) {
		...
		target_btn.attr('disabled', 'disabled');
		art.dialog.confirm('您将给<b>' + oids.length + '</b>位试客返还划算金。<br>返款金额将直接汇入试客互联支付账户，是否继续?', function() {
			var target_btn = $(o);
	...
	order_return : function(o) {
		...
		if (gid > 0) {
			...
			if (oidCount > 0) {
				art.dialog.confirm('您将给<b>' + oidCount + '</b>位试客返还划算金。<br>返款金额将直接汇入试客互联支付账户，是否继续?', function() {
					var target_btn = $(o);
					target_btn.attr('disabled', 'disabled');
