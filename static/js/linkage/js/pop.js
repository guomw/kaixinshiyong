function open_linkage(id,name,container,linkageid) {
	...
	var content = '<div class="linkage-menu"><h6><a href="javascript:;"  onclick="get_parent(this,0)" class="rt"><<返回主菜单</a><span>'+name+'</span> <a href="javascript:;" onclick="get_parent(this)" id="parent_'+id+'" parentid="0"><img src="static/images/icon/old-edit-redo.png" width="16" height="16" alt="返回上一级" /></a></h6><div class="ib-a menu" id="ul_'+id+'">';
...
function get_child(nodeid,nodename,parentid) {
	...
	var url = "index.php?m=Api&c=Linkage&a=index&act=ajax_getlist&parentid="+nodeid+"&keyid="+returnkeyid;
...
function get_parent(obj,parentid) {
	...
	var content = container = '';
	var url = "index.php?m=Api&c=Linkage&a=index&act=ajax_getlist&linkageid="+linkageid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
				container = data.split(',');
...
function get_path(nodeid,returnid) {
	var show_path = '';
	var url = "index.php?m=Api&c=Linkage&a=index&act=ajax_getpath&parentid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$.each(data, function(i,data){ 
				show_path += data+" > ";
...
function set_val(nodename,nodeid) {
	...
	var url = "index.php?m=Api&c=Linkage&a=index&act=ajax_gettopparent&linkageid="+nodeid+"&keyid="+returnkeyid;
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			$("#city").val(data);
		}
