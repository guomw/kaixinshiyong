		...
		init:function() {
			...
			if(site.user.userid > 0) {
				...
				if (site.user.nickname != '') {
						url = '/index.php?m=member&c=profile&a=index';
				}else{
					if (site.user.modelid == 1) {
						/*site.site_root + */
						url = '/index.php?m=member&c=profile&a=infomation';
					}else{
						url = '/index.php?m=member&c=merchant&a=complete';
					};
				}
				str += '<span>'+'<font style="float:left;">您好，</font>'+'<a class="name_hover" href="'+url+'">' + site.nickname + '</a>欢迎来到'+site.webname+'！</span>';
				str += '<span><font style="float:left;">您有未读提醒(</font><a href="/index.php?m=member&c=announce&a=announce&type=1" style="margin-left:5px;">'+site.message+'</a>)条</span>';
