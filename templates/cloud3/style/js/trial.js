...
var trial_detail = (function() {
    ...
    function count_down(sec) {
		...
		var ret = [];
		d && ret.push('<span>'+d+'</span>', '<strong>天</strong>');
		left_h && ret.push('<span>'+time_pad(left_h)+'</span>', '<strong>时</strong>');
		left_m && ret.push('<span>'+time_pad(left_m)+'</span>', '<strong>分</strong>');
		left_s && ret.push('<span>'+time_pad(left_s)+'</span>', '<strong>秒</strong>');
		return ret.join('');
	}
						...
						if (parseInt(site.user.userid) > 0 ){
							$.getJSON(site.site_root+'/index.php?m=order&c=api&a=order_infos&pid='+goods.id,{
								...
								if(ret.status == 1 && (parseInt(ret.info.not_over) > 0 || parseInt(goods.buyer_good_buy_times) <= parseInt(ret.info.over_order))) {
									btn_buy.text('已申请').removeClass('btn_yes').addClass('btn_no').attr('style','background:#D8D8D8;');
                                    // btn_buy.eq(1).text('vip通道').removeClass('btn_no').addClass('btn_yes');
                                    // btn_buy.eq(2).text('积分兑换').removeClass('btn_no').addClass('btn_yes');
									...
									btn_buy.text('免费试用').removeClass('btn_no').addClass('btn_yes');
                                   $('.js_vip').text('vip通道').removeClass('btn_no').addClass('btn_yes');
                                   $('.js_point').text('积分兑换').removeClass('btn_no').addClass('btn_yes');
...
*/	                    	$("#apply_unpass").attr('style','display:');
		...
		talk2seller: function (mss){
            if(mss == 1){
                var talk_html = '<div class="CPM_style CPM_style_2 border_radius_5" style="position: static;width:393px;height: 250px;">';
            }else{
                var talk_html = '<div class="CPM_style CPM_style_2 border_radius_5" style="position: static;width:393px;">';
            }
		...
		trial_url : function(talk_content,bind_taobao,mss) {
            ...
            var buy_succes3 = '<div class="CPM_style CPM_style_2 border_radius_5" style="position: static;">';
            buy_succes3 += '<p class="issue"><a href="'+site.site_root+'/help/?catid=84" target="_blank">如何提高通过率?</a></p><font class="font f_bg_yes font_2">恭喜您，已成功获得免审试用资格，现在可以按照要求<a href="'+site.site_root+'/user/order/?mod=trial">去试用</a></font></div>';
            // 下单方式
//			switch(goods.type) {
//				case 'general': //普通下单
//					buy_succes += '<p class="issue"><a href="'+site.site_root+'/help/?catid=84" target="_blank">如何提高通过率?</a></p><font class="font f_bg_yes font_2">恭喜您，已成功提交申请，请等待商家审核试用资格哦！</font></div>';
