...
<!--
	...
	parent.document.getElementById('center_frame').src = "<?php echo U('public_categorys', array('type' => 'add', 'menuid' => MENUID)) ?>";
...
$(document).ready(function(){
	$('#cat_search').keyup(
		function(){
			...
			if (value.length > 0){
				$.getJSON("<?php echo U('Admin/Category/public_ajax_search') ?>", {catname: value}, function(data){
					if (data.status == 1) {
						var str = '';
						$.each(data.info, function(i,n){
							str += '<li><a href="'+ n.href +'">'+n.catname+'</a></li>';
						});
						$('#search_div').html(str);
						$('#search_div').show();
					} else {
