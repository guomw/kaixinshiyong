...
<div class="subnav">
  <h2 class="title-1 line-x f14 fb blue lh28"><?php echo L('model_manage');?>--「<?php echo $r['name'];?>」<?php echo L('field_manage');?></h2>
	<div class="content-menu ib-a blue line-x"><a class="add fb" href="<?php echo U('add', array('modelid' => $modelid)) ?>"><em><?php echo L('add_field');?></em></a>
　<a class="on" href="<?php echo U('init', array('modelid' => $modelid)) ?>"><em><?php echo L('manage_field');?></em></a><span>|</span><a href="<?php echo U('public_priview', array('modelid' => $modelid)) ?>" target="_blank"><em><?php echo L('priview_modelfield');?></em></a>
	</div>
</div>
<div class="pad-lr-10">
...
<div class="table-list">
    ...
    <tr>
		...
		<td align='center'> <a href="<?php echo U('edit', array('modelid' => $r['modelid'], 'fieldid' => $r['fieldid'])) ?>"><?php echo L('edit');?></a> | 
		<?php if(!in_array($r['field'],$forbid_fields)) { ?>
		<a href="<?php echo U('disabled', array('modelid' => $r['modelid'], 'fieldid' => $r['fieldid'])) ?>"><?php echo $r['disabled'] ? L('field_enabled') : L('field_disabled');?></a>
		<?php } else { ?><font color="#BEBEBE"> <?php echo L('field_disabled');?> </font><?php } ?>|<?php if(!in_array($r['field'],$forbid_delete)) {?> 
		<a href="javascript:confirmurl('<?php echo U('delete', array('modelid' => $r['modelid'], 'fieldid' => $r['fieldid'])) ?>','确定要删除此字段？')"><?php echo L('delete')?></a><?php } else {?><font color="#BEBEBE"> <?php echo L('delete');?></font><?php }?> </td>
	</tr>
	<?php } ?>
    </tbody>
    </table>
