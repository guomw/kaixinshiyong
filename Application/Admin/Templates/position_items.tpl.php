...
?>
<form name="myform" action="<?php echo U('public_item_remove') ?>" method="post">
...
?>   
	...
	<td width=""  align="left"><?php echo $info['data']['title']?> <?php if($info['thumb']!='') {echo '<img src="'.IMG_PATH.'icon/small_img.gif">'; }?></td>
	<td  width="15%" align="center"><?php echo $info['catname']?></td>
	<td width="15%" align="center"><?php echo date('Y-m-d H:i:s',$info['data']['inputtime'])?></td>
	<td width="15%" align="center"><a href="<?php echo U('Document/Index/show', array('catid' => $info['catid'], 'id' => $info['id'])) ?>" target="_blank"><?php echo L('posid_item_view')?></a> | <a onclick="javascript:openwinx('<?php echo U('Document/Document/edit', array('catid' => $info['catid'], 'id' => $info['id'])) ?>','')" href="javascript:;"><?php echo L('posid_item_edit');?></a> | <a href="javascript:item_manage(<?php echo $info['id']?>,<?php echo $info['posid']?>, <?php echo $info['modelid']?>,'<?php echo $info['title']?>')"><?php echo L('posid_item_manage')?></a>
	</td>
	</tr>
<?php 
	}
...
?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('select_all')?>/<?php echo L('cancel')?></label> <input type="button" class="button" value="<?php echo L('listorder')?>" onclick="myform.action='<?php echo U('public_item_listorder', array('dosubmit' => true)) ?>';myform.submit();"/> <input type="submit" class="button" name="dosubmit" value="<?php echo L('posid_item_remove')?>" /> </div></div>
