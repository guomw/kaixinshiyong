...
<table cellpadding="2" cellspacing="1" class="table_form" width="100%" id="goods_albums"> 
	<tr>
		...
		<td>
			<select name="image[type]">
				<option value="1" <?php if($type == 1){?>selected<?php }?>>首页幻灯</option>
				<option value="2" <?php if($type == 2){?>selected<?php }?>>试用报告幻灯</option>
			</select>
		</td>
	</tr>
	<tr>
		<th width="15%">幻灯标题 :</th>
		<td><input type="text" name="image[title]" id="title" size="20" value="<?php echo $title;?>"/></td>
	</tr>
	<tr>
		<th>幻灯大图：</th>
		<td>
			<div class="sub_btn" name="uploadify" id="img1">
				<img src="<?php echo $image;?>" alt="" width="100" height="50" onerror="javascript:this.src='<?php echo THEME_STYLE_PATH;?>style/images/signIn_14.jpg'"/>
	...
	<tr>
		...
		<td><input type="color" id="color" name="image[color]" value="<?php echo $color;?>" size="20"></td>
	</tr>
	
	<tr>
		<th>开始时间：</th>
		<td>
			<?php echo $form::date('image[starttime]',dgmdate($starttime,'Y-m-d H:i:s'),1)?>
		</td>
	</tr>
	<tr>
		<th>结束时间：</th>
		<td>
			<?php echo $form::date('image[endtime]', dgmdate($endtime,'Y-m-d H:i:s'),1)?>
		</td>
	</tr>
		<tr>
		<th>链接地址：</th>
		<td>
			<input type="text" name="image[url]" id="url" size="20" value="<?php echo $url;?>" />
		</td>
	</tr>
	<tr>
		<th></th>
