...
Class CategoryController extends InitController {
        ...
        if(!empty($result)) {
            foreach($result as $r) {
                ...
                if(!$show_detail) {
                    ...
                    $r['parentid'] = 0;
                    $r['str_manage'] .= '<a href="'.U('Category/init', array('parentid' => $r['catid'], 'menuid' => MENUID, 'type' => $r['menuid'], 'fromhash' => session('FROMHASH'))).'">管理子栏目</a> | ';
                }
                $r['str_manage'] .= '<a href="'.U('Category/add', array('parentid' => $r['catid'], 'menuid' => MENUID, 'type' => $r['menuid'], 'fromhash' => session('FROMHASH'))).'">添加子栏目</a> | ';
                
                $r['str_manage'] .= '<a href="'.U('Category/edit', array('catid' => $r['catid'], 'menuid' => MENUID, 'type' => $r['menuid'], 'fromhash' => session('FROMHASH'))).'">编辑</a> | <a href="javascript:confirmurl(\''.U('Category/delete', array('catid' => $r['catid'], 'menuid' => MENUID, 'type' => $r['menuid'], 'fromhash' => session('FROMHASH'))).'\',\'您确定要删除这个栏目吗？\')">删除</a> | <a href="'.U('Category/remove', array('catid' => $r['catid'], 'menuid' => MENUID, 'fromhash' => session('FROMHASH'))).'">批量移动</a>';
    ...
    public function add($type = 0) {
        ...
        if (submitcheck('dosubmit')) {
            ...
            switch ($type) {
                ...
                // 标准栏目
                default:
                    if ($info['modelid'] < 1) $this->error('请选择所属模型');
                    $model_info = $this->model_db->detail($info['modelid']);
                    if (empty($model_info) || $model_info['disabled'] == 1){
                        $this->error('您选择的模型不存在或已被禁用');
                    }
                    break;
            }
            $info['type'] = $type;
    ...
    */
    ...
    public function public_category_load($modelid = 0) {
        ...
        $modelid = (int) $modelid;
        $category = $form::select_category(0, 'name="info[catid]"', '请选择栏目', $modelid);
        echo $category;
        exit();
    }
