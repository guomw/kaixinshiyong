	...
	<div class="col-right">
    	<div class="col-1">
        	<div class="content pad-6">
        		...
        		<?php echo $this->task_status[$rs['status']]?>
          </div>
        </div>
    </div>
...
<table width="100%" cellspacing="0" class="table_form">
	...
	<tr>
      ...
      <td><input type="text" name="info[answer]" id="answer" value="<?php echo $rs['answer'];?>" class="input-text" style="width:300px" /></td>
    </tr>    
	...
	<tr>
      ...
      <td><input type="text" name="info[keyword]" class="input-text"  id="keyword" style="width:300px" value="<?php echo $rs['keyword'];?>" />&nbsp;请输入商品关键字，多个用英文逗号（,）分割</td>
    </tr>
    
    <tr>
      <th>排序要求：<font color="red">*</font></th>
      <td>
      	<label><input type="radio" value="1" name="info[sort]" <?php if($rs['sort'] == 1){?>checked <?php }?>/>综合</label>
      	<label><input type="radio" value="2" name="info[sort]" <?php if($rs['sort'] == 2){?>checked <?php }?> />人气</label>
      	<label><input type="radio" value="3" name="info[sort]" <?php if($rs['sort'] == 3){?>checked <?php }?>/>销量</label>
      	<label><input type="radio" value="4" name="info[sort]" <?php if($rs['sort'] == 4){?>checked <?php }?>/>信用</label>
      	<label><input type="radio" value="5" name="info[sort]" <?php if($rs['sort'] == 5){?>checked <?php }?>/>最新</label>
      	<label><input type="radio" value="6" name="info[sort]" <?php if($rs['sort'] == 6){?>checked <?php }?>/>价格</label>
      </td>
    </tr>
	...
	<tr>
		...
		<td><input type="text" name="info[goods_number]" id="goods_number" value="<?php echo $rs['goods_number'];?>" />份</td>
	</tr>
	
	<tr>
		<th>每份佣金：<font color="red">*</font></th>
		<td><input type="text" name="info[goods_price]" id="goods_price" value="<?php echo $rs['goods_price'];?>"/>美元</td>
	</tr>	
	...
	<tr>
		...
		<td>
			...
			<div class="picBut cu"><a herf="javascript:void(0);" onclick="javascript:flashupload('goods_albums', '附件上传','goods_albums',change_images,'20,,','product','1','c7ca0ab7bba2e9e20f87b86214ff27c1')"> 选择图片 </a></div>
		</td>
	</tr>		
    </tbody>
</table>
