...
class image {
    ...
    function __construct($thumb_enable = 0) {
    	$this->thumb_enable = $thumb_enable;

    	$this->watermark_enable = C('WATERMARK_ENABLE');
    	
    	$this->set(C('WATERMARK_IMG'), C('WATERMARK_POS'), C('WATERMARK_MINWIDTH'), C('WATERMARK_MINHEIGHT'), C('WATERMARK_QUALITY'), C('WATERMARK_PCT'));
    }
    ...
    /* 图片添加水印 */
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'tpcms',$w_font = 8, $w_color = '#ff0000') {
		$w_pos = $w_pos ? $w_pos : $this->w_pos;
		$w_img = $w_img ? $w_img : $this->w_img;
		if(!$this->watermark_enable || !$this->check($source)) return false;
		if(!$target) $target = $source;
		$w_img = SITE_PATH.'/static/images/water/'.$w_img;
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, COMMON_PATH.'/Data/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
