...
class attachment {
	...
	function __construct($module='', $catid = 0, $upload_dir = '') {
		$this->catid = intval($catid);
		$this->module = $module ? $module : 'document';	
		$this->att_db = D('Attachment');		
		$this->upload_root = SITE_PATH.'/uploadfile/';
	...
	function upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0,$thumb_setting = array(), $watermark_enable = 1) {
		...
		foreach($uploadfiles as $k=>$file) {
			...
			$filepath = preg_replace(daddslashes("|^".$this->upload_root."|"), "", $savefile);
	...
	function download($field, $value,$watermark = '0',$ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '')
	{
		global $image_d;
		$this->field = $field;
		$upload_url = __ROOT__.'uploadfile/';
		$dir = date('Y/md/');
	...
	function add($uploadedfile) {
		...
		$uploadedfile['uploadip'] = get_client_ip();
		$uploadedfile['status'] = C('attachment_stat') ? 0 : 1;
		...
		$aid = $this->att_db->add($uploadedfile);
	...
	*/
	...
	function is_allow_upload() {
		...
		$starttime = NOW_TIME-86400;
		return ($uploads < C('upload_maxsize'));
	}
