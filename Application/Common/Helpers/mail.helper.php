...
function sendmail($toemail, $subject, $message, $from='',$cfg = array(), $sitename='') {
	...
	if(!$mail['auth_username'] || !$mail['auth_password']){
		return false;
	}
	//邮件头的分隔符
	...
	if(!$fp = fsockopen($mail['server'], $mail['port'], $errno, $errstr, 30)) {
		Think\Log::record('SMTP', "($mail[server]:$mail[port]) CONNECT - Unable to connect to the SMTP server", 0);
		return false;
	}
	...
	if(substr($lastmessage, 0, 3) != '220') {
		Think\Log::record('SMTP', "$mail[server]:$mail[port] CONNECT - $lastmessage", 0);
		return false;
	}
	...
	if(substr($lastmessage, 0, 3) != 220 && substr($lastmessage, 0, 3) != 250) {
		Think\Log::record('SMTP', "($mail[server]:$mail[port]) HELO/EHLO - $lastmessage", 0);
		return false;
	}
	...
	if($mail['auth']) {
		...
		if(substr($lastmessage, 0, 3) != 334) {
			Think\Log::record('SMTP', "($mail[server]:$mail[port]) AUTH LOGIN - $lastmessage", 0);
			return false;
		}
		...
		$lastmessage = fgets($fp, 512);
		if(substr($lastmessage, 0, 3) != 334) {
			Think\Log::record('SMTP', "($mail[server]:$mail[port]) USERNAME - $lastmessage", 0);
			return false;
		}
		...
		if(substr($lastmessage, 0, 3) != 235) {
			Think\Log::record('SMTP', "($mail[server]:$mail[port]) PASSWORD - $lastmessage", 0);
			return false;
		}
	...
	if(substr($lastmessage, 0, 3) != 250) {
		...
		$lastmessage = fgets($fp, 512);
		if(substr($lastmessage, 0, 3) != 250) {
			Think\Log::record('SMTP', "($mail[server]:$mail[port]) MAIL FROM - $lastmessage", 0);
			return false;
		}
	}
	...
	if(substr($lastmessage, 0, 3) != 250) {
		fputs($fp, "RCPT TO: <".preg_replace("/.*\<(.+?)\>.*/", "\\1", $toemail).">\r\n");
		$lastmessage = fgets($fp, 512);
		Think\Log::record('SMTP', "($mail[server]:$mail[port]) RCPT TO - $lastmessage", 0);
		return false;
	}
	...
	if(substr($lastmessage, 0, 3) != 354) {
		Think\Log::record('SMTP', "($mail[server]:$mail[port]) DATA - $lastmessage", 0);
		return false;
	}
	...
	$lastmessage = fgets($fp, 512);
	if(substr($lastmessage, 0, 3) != 250) {
		Think\Log::record('SMTP', "($mail[server]:$mail[port]) END - $lastmessage", 0);
	}
	fputs($fp, "QUIT\r\n");
	return true;
}
