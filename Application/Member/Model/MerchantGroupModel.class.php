...
use Think\Model;
Class MerchantGroupModel extends Model {
	/*自动验证*/
	protected $_validate = array (
		// array(验证字段1,验证规则,错误提示,[验证条件,附加规则,验证时间]),
		array('name', 'require', '商家类型不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
		array('name', '', '商家类型名称已存在', self::MUST_VALIDATE, 'unique', self::MODEL_BOTH),
	);
	...
	public function build_cache() {
		...
		setcache('merchant_group', $result,'member');
		return $result;
	}
}