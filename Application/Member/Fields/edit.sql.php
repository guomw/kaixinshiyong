...
switch($field_type) {
	...
	case 'tinyint':
		sqlexecute("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '0'");
	break;
	...
	case 'smallint':
		$minnumber = intval($minnumber);
		sqlexecute("ALTER TABLE `$tablename` CHANGE `$oldfield` `$field` SMALLINT ".($minnumber >= 0 ? 'UNSIGNED' : '')." NOT NULL");
	break;
