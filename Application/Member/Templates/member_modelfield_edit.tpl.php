...
$(function(){
	...
	$("#field").formValidator({onshow:"<?php echo L('input').L('fieldname')?>",onfocus:"<?php echo L('fieldname').L('between_1_to_20')?>"}).regexValidator({regexp:"^[a-zA-Z]{1}([a-zA-Z0-9]|[_]){0,19}$",onerror:"<?php echo L('filedname_incorrect')?>"}).inputValidator({min:1,max:20,onerror:"<?php echo L('fieldname').L('between_1_to_20')?>"}).ajaxValidator({
	    type : "get",
		url : "<?php echo U('public_checkfield');?>",
		data : {modelid : '<?php echo $modelid; ?>', 'oldfield' : '<?php echo $field;?>'},
		datatype : "html",
		cached:false,
		async:'true',
...
<form name="myform" id="myform" action="<?php echo U('edit'); ?>" method="post">
...
<fieldset>
	...
	<?php echo $form::select($fields,$formtype,'name="info[formtype]" id="formtype" onchange="javascript:field_setting(this.value);" disabled',L('filedtype_need'));?>
		...
		<tr> 
		  ...
		  <td><textarea name="info[tips]" rows="2" cols="20" id="tips" style="height:40px; width:80%"><?php echo dhtmlspecialchars($tips)?></textarea></td>
		</tr>
		...
		<tr> 
		  ...
		  <td><input type="text" name="info[formattribute]" size="50" class="input-text" value="<?php echo dhtmlspecialchars($formattribute);?>"></td>
		</tr>
		<tr> 
		  <th><strong><?php echo L('form_css')?></strong><br /><?php echo L('user_form_css')?></th>
		  <td><input type="text" name="info[css]" size="10" class="input-text" value="<?php echo dhtmlspecialchars($css);?>"></td>
		</tr>
		<tr> 
