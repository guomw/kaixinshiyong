...
$(function(){
	...
	$("#modelname").formValidator({onshow:"<?php echo L('input').L('model_name')?>",onfocus:"<?php echo L('model_name').L('between_2_to_20')?>"}).inputValidator({min:2,max:20,onerror:"<?php echo L('model_name').L('between_2_to_20')?>"}).regexValidator({regexp:"ps_username",datatype:"enum",onerror:"<?php echo L('model_name').L('format_incorrect')?>"}).ajaxValidator({
	    type : "get",
		url : "<?php echo U('Member/MemberModel/public_checkmodelname'); ?>",
		datatype : "html",
		async:'false',
	...
	$("#tablename").formValidator({onshow:"<?php echo L('input').L('table_name')?>",onfocus:"<?php echo L('table_name').L('format_incorrect')?>",oncorrect:"<?php echo L('table_name').L('format_right')?>"}).inputValidator({min:2,max:20,onerror:"<?php echo L('table_name').L('between_2_to_20')?>"}).regexValidator({regexp:"letter_l",datatype:"enum",onerror:"<?php echo L('table_name').L('format_incorrect')?>"}).ajaxValidator({
	    type : "get",
		url : "<?php echo U('Member/MemberModel/public_checktablename'); ?>",
		datatype : "html",
		async:'false',
		success : function(data){	
            if(data == 0) {
...
<form name="myform" action="<?php echo U('add');?>" method="post" id="myform" enctype="multipart/form-data">
<fieldset>
	...
	<table width="100%" class="table_form">
		...
		<tr>
			...
			<td>
			<?php echo C('DB_PREFIX');?>member_<input type="text" name="info[tablename]" value="" class="input-text" id="tablename" size="16"></input>
			</td>
		</tr>
		<tr>
